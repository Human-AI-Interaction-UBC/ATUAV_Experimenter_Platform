...
class EyetrackerCreatedCallbackHandler:
    def __init__(self, mycls, callback, *args, **kwargs):
        self.eyetracker_class=mycls
        self.callback=callback
        self.args=args
        self.kwargs=kwargs

    def trampoline(self, error, message_passer):
        if error:
            self.callback(error, None, *self.args, **self.kwargs)
        else:
            self.callback(0, self.eyetracker_class(message_passer), *self.args, **self.kwargs)
...
class BasicEyetracker(object):
    ...
    class ResponseHandlerFunctor(object):
        ...
        def __call__(self, opcode, error, payload):
            ...
            try:
                ...
                print "Exception during response_callback: %s" %(ex)
    ...
    class ChannelHandlerFunctor(object):
        ...
        def __call__(self, opcode, error, payload):
            try:
                ...
                print "Exception during ChannelHandlerFunctor.__call__(): %s" %(ex)
    ...
    def create_async(cls, mainloop, eyetracker_info, callback, *args, **kwargs):
        ...
        BasicEyetracker.eyetracker_created_callback_handler=EyetrackerCreatedCallbackHandler(cls, callback, *args, **kwargs)    
        tetio.get_message_passer(fi, ml.mainloop, BasicEyetracker.eyetracker_created_callback_handler.trampoline)
