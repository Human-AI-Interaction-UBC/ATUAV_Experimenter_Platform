...
import struct
from tobii.eye_tracking_io.types import (Point2D, Node)
...
class OneCalibrationSample(object):
    def __init__(self, map_point, status, quality):
        self.map_point = map_point
        self.status = status
        self.quality = quality
    
    def __str__(self):
        return "%s status: %d" % (self.map_point, self.status)
    
    @staticmethod
    def _extract(blob, offset):
        (x, y, s, q) = unpack_from("<fflf", blob, offset)
        return OneCalibrationSample(Point2D(x, y), s, q)
