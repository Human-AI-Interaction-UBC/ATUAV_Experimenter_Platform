    ...
    def post(self):
        ...
        if(int(q1)==1):
            ...
            if params.BETWEEN:
                ...
                print(cond_type)
                self.application.cond_types = [cond_type]
            else:
                self.application.cond_types = ['underline', 'branching']
...
class QuestionnaireHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        noofMMD = len(self.application.conditions[self.application.cond_types[self.application.cond_index]])
    ...
    def post(self):
        ...
        self.application.conn.execute('INSERT INTO Study_progress VALUES (?,?,?,?)', [self.application.cur_user, 'mmd',
                                                                                      self.application.cur_mmd, str(datetime.datetime.now().time())])
        self.application.conn.commit()
...
class MMDHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        curCond = self.application.conditions[self.application.cond_types[self.application.cond_index]]
        if self.application.mmd_index<len(curCond):
            self.application.cur_mmd = curCond[self.application.mmd_index]
...
class SubcondHandler(tornado.web.RequestHandler):
    def get(self):
        self.render("subcond.html")
